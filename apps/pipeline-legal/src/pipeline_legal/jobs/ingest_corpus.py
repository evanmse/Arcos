"""ARCOS — Cloud Run Job: ingest one or more regulations end-to-end.

Usage (locally):
    python -m pipeline_legal.jobs.ingest_corpus --regulation dora
    python -m pipeline_legal.jobs.ingest_corpus --all

Environment:
    ARCOS_GCP_PROJECT_ID, ARCOS_GCP_REGION, ARCOS_RAW_LEGAL_BUCKET,
    ARCOS_PG_*, ARCOS_VECTOR_SEARCH_INDEX_LEGAL, ARCOS_NEO4J_*

Pub/Sub: at the end of a successful ingestion, publishes a message on
ARCOS_PUBSUB_TOPIC_LEGAL_UPDATES with { regulation_id, celex, chunks, obligations, run_id }.
"""
...
from pipeline_legal.chunker import chunk_articles
from pipeline_legal.corpus import CORPUS, get_regulation
from pipeline_legal.crawlers.eurlex import GCSWriter, crawl_eurlex
from pipeline_legal.embedder import VertexEmbeddingClient, embed_chunks
from pipeline_legal.extractor import VertexGeminiClient, extract_obligations
from pipeline_legal.indexer import (
    Neo4jWriter,
    NoopGraphWriter,
    TripleIndexer,
    VertexVectorSearchUpserter,
)
from pipeline_legal.logging_setup import configure_logging, get_logger
from pipeline_legal.models import Article, Regulation
from pipeline_legal.parsers.html_parser import parse_eurlex_html
from pipeline_legal.settings import Settings, get_settings

log = get_logger(__name__)


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Ingest a regulation into ARCOS legal corpus.")
